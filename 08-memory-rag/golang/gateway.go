...
func (g *Gateway) dispatch(raw ACPMessage) {
	...
	// Recover from any tool panic so the client gets an error instead of silence.
	defer func() {
		if r := recover(); r != nil {
			fmt.Fprintf(os.Stderr, "[gateway] panic in dispatch: %v\n", r)
			adapter.send(AgentReply{
				Type: "error", ID: msg.ID, SessionID: msg.SessionID,
				Channel: msg.Channel, Content: fmt.Sprintf("internal error: %v", r),
			})
		}
	}()
