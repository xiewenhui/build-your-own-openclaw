...
// Relative paths are resolved against workDir (not the process CWD).
// Defense 1: filepath.Join + Clean removes all ".." components; prefix check blocks traversal.
func canonicalize(path, workDir string) (string, error) {
	...
	if filepath.IsAbs(path) {
		...
		abs = filepath.Join(workAbs, path)
	}
...
func registerToolsForMode(mode string, pool *SandboxPool, hitl HITLConfirmer, workDir string) {
	switch mode {
	case "full":
		registerFullSandboxTools(pool)
	default:
		registerHostModeTools(hitl, workDir)
	}
}
...
func registerHostModeTools(hitl HITLConfirmer, workDir string) {
...
func buildSystemPromptWithMemory(agentMemories, kbSnippets string) string {
	today := time.Now().Format("2006-01-02")
	prompt := `You are an AI assistant named xclaw.
Today's date: ` + today + `
...
func registerMemoryTools(store MemoryStore) {
	registerTool(ToolDefinition{
		Name:        "memory_save",
		...
		emb, err := Embed(context.Background(), content)
		if err != nil {
			return "", fmt.Errorf("embed: %w", err)
		}
		id, err := store.Save(context.Background(), MemoryEntry{
			SessionID: sessionID,
			Source:    SourceAgent,
			Content:   content,
			Embedding: emb,
	...
	registerTool(ToolDefinition{
		...
		emb, err := Embed(context.Background(), params["query"])
		if err != nil {
			return "", fmt.Errorf("embed: %w", err)
		}
		results, err := store.Search(context.Background(), emb, topK, SearchFilter{Source: SourceAgent})
		if err != nil {
			return "", err
		}
		if len(results) == 0 {
		...
		for i, r := range results {
			lines = append(lines, fmt.Sprintf("[%d] %s", i+1, r.Content))
		}
...
func registerKBTools(store MemoryStore, workDir string) {
	registerTool(ToolDefinition{
		Name:        "kb_index",
		...
		Required: []string{"path", "doc_id"},
	}, func(_ string, params map[string]string) (string, error) {
		abs := filepath.Join(workDir, params["path"])
		abs = filepath.Clean(abs)
		if !strings.HasPrefix(abs, filepath.Clean(workDir)) {
			return "error: path outside workspace", nil
		}
		count, err := IndexDocument(context.Background(), abs, params["doc_id"], store)
		if err != nil {
			return "", err
		}
		return fmt.Sprintf("indexed %d chunks from %s", count, params["path"]), nil
	})
	...
	registerTool(ToolDefinition{
		...
		filter := SearchFilter{Source: SourceKB}
		if d := params["doc_id"]; d != "" {
			filter.DocID = d
		}
		results, err := store.Search(context.Background(), emb, topK, filter)
		if err != nil {
			return "", err
		}
		if len(results) == 0 {
		...
		var parts []string
		for i, r := range results {
			parts = append(parts, fmt.Sprintf("[%d] (doc: %s)\n%s", i+1, r.DocID, r.Content))
		}
