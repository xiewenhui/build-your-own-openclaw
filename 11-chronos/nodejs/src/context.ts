import type { Message, ContentBlock, Provider } from './providers/types.ts';

// ── Token estimation ─────────────────────────────────────────────────────────

// Images cost roughly 1000 tokens each; text is ~4 chars/token.
  ...
  const summaryRequest: Message[] = [
    {
      ...
      content:
        ...
        toSummarize.map(m => `${m.role}: ${typeof m.content === 'string' ? m.content : '[image]'}`).join('\n'),
    },
  ];
