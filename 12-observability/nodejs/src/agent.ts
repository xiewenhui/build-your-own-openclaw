...
export class Agent {
  ...
  private async _handleInner(msg: ACPMessage, onDelta: (token: string) => void): Promise<string> {
    ...
    if (!this.sessions.has(msg.sessionId)) {
      ...
      if (this.db) {
        ...
        if (status !== null) {
          const loaded = this.db.loadMessages(msg.sessionId, this.systemPromptOverride ?? buildSystemPrompt(undefined, undefined, msg.content));
          this.sessions.set(msg.sessionId, loaded);
          log(`[agent] [${msg.sessionId}] restored ${loaded.length} messages from DB (status: ${status})`);
        }
      }
      if (!this.sessions.has(msg.sessionId)) {
        this.sessions.set(msg.sessionId, [{ role: 'system', content: this.systemPromptOverride ?? buildSystemPrompt(undefined, undefined, msg.content) }]);
      }
    }
    ...
    if (this.memoryStore) {
      try {
        ...
        const sysPrompt = buildSystemPrompt(memLines || undefined, kbLines || undefined, msg.content);
    ...
    while (true) {
      ...
      try {
        const result = await traceSpan(
          'llm.call',
          { type: 'LLM_CALL', model: this.providerChain[0] ?? 'unknown', sessionId: msg.sessionId },
          () => streamWithFallback(messages, this.providerChain, (token) => {
            buffer.push(token);
          }),
        );
        reply = result.reply;
      ...
      if (toolCall && typeof toolCall['action'] === 'string') {
        ...
        if (tool) {
          ...
          (global as any).__toolHook?.(action);
          try {
            const output = await traceSpan(
              'tool.exec',
              { toolName: action, sessionId: msg.sessionId },
              () => tool.execute(msg.sessionId, params, onDelta),
            );
  ...
  private handleCommand(msg: ACPMessage): string | null {
    ...
    // /skills — list all registered skills and their availability
    if (content === '/skills') {
      const statuses = globalSkillRegistry.listStatus();
      if (statuses.length === 0) return 'No skills registered.';
      let out = 'Available skills:\n';
      for (const { skill, ok, missing } of statuses) {
        const fm = skill.frontmatter;
        const emoji = fm.metadata?.openclaw?.emoji ?? '  ';
        const auto = fm['user-invocable'] === false ? ' (auto-injected)' : '';
        if (ok) {
          out += `  ${emoji} ✅ ${fm.name.padEnd(20)} — ${fm.description}${auto}\n`;
        } else {
          const hints = (fm.metadata?.openclaw?.install ?? [])
            .map((i: any) => i.label)
            .join(' / ');
          out += `  ${emoji} ❌ ${fm.name.padEnd(20)} — requires: ${missing.join(', ')}${hints ? `  (${hints})` : ''}\n`;
        }
      }
      return out;
    }
...
3. **步数硬限制**：最多执行 ${maxSteps} 次工具调用。超出限制时，立即停止并输出一行简短说明。不要循环重试。
...
export function buildSystemPrompt(agentMemories?: string, kbSnippets?: string, userMessage?: string): string {
...
**CRITICAL RULE — Skills Execution (PRIORITY 1):**
When an "Available Skills" section appears at the bottom of this prompt, you MUST NOT under any circumstances delegate, mention, or send these tasks to "coder" or any other worker.
You MUST execute the provided commands yourself IMMEDIATELY by outputting a JSON tool call using the shell tool.
- NEVER output the command as plain text.
- NEVER let a worker handle it.
Example: {"action": "shell", "command": "node C:\\path\\to\\script.js memory"}
