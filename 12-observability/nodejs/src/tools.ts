...
// Track which tools were registered by plugins so they always appear in the Orchestrator prompt.
// (ORCHESTRATOR_TOOLS whitelist covers built-ins; plugin tools are additive.)
export const pluginToolNames = new Set<string>();
...
// Plugin tools are always included (they are explicitly opt-in by the plugin author).
const ORCHESTRATOR_TOOLS = new Set([
  'delegate', 'debate', 'pipeline',
  'view_file', 'list_dir',
  'memory_save', 'memory_search', 'kb_search',
  'shell',  // skills inject runnable scripts — Orchestrator executes them directly
  'notify', // CHRONOS MODE: alert on anomaly
]);

export function buildOrchestratorToolsPrompt(): string {
  return [...toolRegistry.values()]
    .filter(({ definition }) => ORCHESTRATOR_TOOLS.has(definition.name) || pluginToolNames.has(definition.name))
    .map(({ definition: d }) => {
...
// QQ token cache for notify tool (independent of channels/qq.ts to avoid shared mutable state)
let qqNotifyTokenCache: { token: string; expiresAt: number } | null = null;

async function getQQNotifyToken(appId: string, secret: string): Promise<string> {
  if (qqNotifyTokenCache && Date.now() < qqNotifyTokenCache.expiresAt - 60_000) {
    return qqNotifyTokenCache.token;
  }
  ...
  qqNotifyTokenCache = { token: data.access_token, expiresAt: Date.now() + data.expires_in * 1000 };
...
function registerHostModeTools(hitl: HITLConfirmer, cfg: Config): void {
  ...
  // shell — host mode: run command on host, no sandbox isolation
  registerTool(
    {
      name: 'shell',
      description: 'Execute a shell command on the host and return stdout+stderr. Use this to run skill scripts.',
      parameters: {
        type: 'object',
    ...
    async (_sessionId, params) => {
      if (!await hitl.confirm(`shell: ${params['command']}`, '', false)) {
        throw new Error('user denied');
      }
  ...
  // notify — CHRONOS MODE: send anomaly alert via Feishu / QQ / stdout fallback
  // Priority: 1) FEISHU_WEBHOOK_URL  2) QQ_APP_ID+QQ_CLIENT_SECRET+QQ_NOTIFY_OPENID  3) stdout
  registerTool(
    {
      name: 'notify',
    ...
    async (_sessionId, params) => {
      const title   = params['title']   ?? '(无标题)';
      const message = params['message'] ?? '';
      const level   = params['level']   ?? 'INFO';
      const emoji   = level === 'CRITICAL' ? '🚨' : level === 'WARNING' ? '⚠️' : 'ℹ️';
      const time    = new Date().toLocaleString();
      const text    = `${emoji} [xclaw 巡检] [${level}] ${title}\n${message}\n时间：${time}`;

      // ── Priority 1: Feishu Webhook ──────────────────────────────────────────
      ...
      if (feishuUrl) {
        ...
        const payload = {
          ...
          card: {
            ...
            elements: [
              { tag: 'markdown', content: `**级别:** ${level}　**时间:** ${time}` },
              { tag: 'hr' },
      ...
      // ── Priority 2: QQ proactive push ──────────────────────────────────────
      // Distinct from channels/qq.ts reactive replies (which need replyCtx + msg_id).
      // Here the target openid is configured via env var; msg_id is omitted.
      const qqAppId  = process.env['QQ_APP_ID'];
      const qqSecret = process.env['QQ_CLIENT_SECRET'];
      const qqOpenid = process.env['QQ_NOTIFY_OPENID'];
      const qqType   = process.env['QQ_NOTIFY_TYPE'] ?? 'c2c';
